...
for (const adapter of ADAPTERS) {
  windowedTest(`adapters/${adapter.id} renders alert dialog`, async ({ page, demoPage: d }) => {
    ...
    await page.getByRole("combobox", { name: "Render with" }).click();
    await page.getByRole("option", { name: adapter.label, exact: true }).click();

    await d.clickButtonInCard("the-basics", "alert-dialog", "Show alert dialog");
    await expect(page.getByText("This is an important alert message that you should read!")).toBeVisible();

    await page.getByRole("button", { name: /Got it|OK/i }).click();
    await expect(page.getByText("This is an important alert message that you should read!")).toBeHidden();
  });
}
...
test.describe("adapter switcher", () => {
  test("persists adapter choice in localStorage", async ({ page }) => {
    ...
    await page.reload();
    await expect(page.getByRole("combobox", { name: "Render with" })).toHaveText(/Base UI/);
  });
});
