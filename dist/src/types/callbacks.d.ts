...
type ResolveActionId<TActionId extends string> = [TActionId] extends [never] ? BuiltInActionId : BuiltInActionId | TActionId;
...
type ResolveAction<TActionId extends string> = [TActionId] extends [never] ? "okClicked" | "cancelClicked" : "okClicked" | "cancelClicked" | `${TActionId}Clicked`;
/**
 ...
 * @template TActionId - Union of custom action ids. Default (never) = built-in "ok" | "cancel" only. Specify e.g. "draft" | "delete" to add custom ids.
 */
export interface DialogActionEvent<TResolveValue = unknown, TActionId extends string = never> extends DialogCloseEvent<TResolveValue, TActionId> {
    reason: "action";
    action: ResolveAction<TActionId>;
    actionId: ResolveActionId<TActionId>;
    buttonText?: string;
...
export type UseDialogOn = <K extends keyof DialogistEventMap>(event: K, handler: (payload?: DialogistEventMap[K]) => void) => () => void;
/** `dialog.off` for a `useDialog` instance — same contract as {@link UseDialogOn}. */
export type UseDialogOff = <K extends keyof DialogistEventMap>(event: K, handler: (payload?: DialogistEventMap[K]) => void) => void;
