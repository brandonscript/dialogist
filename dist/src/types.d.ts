...
export type { DialogActionEvent, DialogCallbackRegistration, DialogCallbacksTriggerFn, DialogCloseEvent, DialogEventName, DialogistEventMap, DialogistEventMapWith, UseDialogEmit, UseDialogOff, UseDialogOn, } from "./types/callbacks";
 ...
 * });
 */
export type DialogConfigWithTypedMessage<P extends Record<string, unknown>> = Partial<Omit<BaseDialogConfig, "message" | "content" | "props">> & ({
    message: ComponentType<P>;
    content?: never;
    props: P;
} | {
...
export type DialogCloseResolver<TResolveValue = unknown> = Omit<DialogCloseEvent<TResolveValue, string>, "ok" | "cancelled" | "blocked" | "resolveValue"> & {
...
export type DialogActionsInput<T = unknown> = DialogActionProps<T>[] | (DialogActionProps<T> | DialogActionProps<T>[])[];
/**
...
export type DialogOpenConfig = DialogConfig & Partial<DialogOpenInternalFields> & {
    ownerToken?: symbol;
};
