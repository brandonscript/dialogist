...
get_package_dirs() {
  # For single package, just return current directory
  echo "."
}
...
update_workspace_dependencies() {
  ...
  # No workspace dependencies for single package
...
revert_workspace_dependencies() {
  ...
  for package_dir in "${consumer_packages[@]}"; do
    ...
    if [[ -f "$package_json" ]]; then
      # Force @dialogist/core to workspace:* regardless of current state
      if jq -e '.dependencies."@dialogist/core"' "$package_json" >/dev/null 2>&1; then
